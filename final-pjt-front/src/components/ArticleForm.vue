    ...
    <div class="container bg-light deco-container p-5">
      <div class="d-flex flex-column align-items-center mb-3">
        ...
        <form @submit.prevent="search()" class="text-center">
          <input id="search-movie-input" type="text" v-model="searchKeyword" placeholder="Enter Movie Title" disabled required />
      ...
      <form @submit.prevent="makeArticle()" >
        <!-- movieId 받기 -->
        ...
        <div class="text-right mt-3">
          <button class="submit-button">{{ action }}</button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      searchKeyword: this.article.movie.title,
      result: [],
      articleData: {
        movie_pk: this.article.movie.id,
        title: this.article.title,
        choices: this.article.choices,
      }
    } 
  },
  methods: {
    ...mapActions(['articleNew', 'editArticle',]),
    makeArticle() {
      if (this.action === 'create') {
        this.articleNew(this.articleData)
